aft_bart_np_ps_HTE <- function(){
  ...
  # Superlearner to estimate PS
  
  SL.library <- c("SL.glmnet", "SL.glm", "SL.gam")
  mydata_X <- mydata$X %>% as_tibble()
  
  y_treatment <- mydata_X$W
  
  X <- mydata_X %>% 
    select(-W, -Time, -Event) %>% as.data.frame()
  
  mydata_X_superlearner <- SuperLearner(Y = y_treatment, X = X, SL.library = SL.library,
                                                 verbose = TRUE, method = "method.NNLS", family = binomial())
  mydata_X_superlearner_predict <- predict(mydata_X_superlearner)
  
  mydata_X_superlearner_ps <- ifelse(y_treatment == 0, 1-as.numeric(mydata_X_superlearner_predict$pred), as.numeric(mydata_X_superlearner_predict$pred))
  
  # Create counterfactual data
  mydata_trt_1 <- mydata$X %>% as_tibble() %>%  
    mutate(ps = mydata_X_superlearner_ps) %>% 
    filter(W == 1) %>% 
    select(-W) 
