...
DR_DL_HTE <- function(){
  ...
  for (i in 1:length(time.point)){
    targets_1 = create.imp.dr.brier.km(obs = mydata_trt_1$Time, delta = mydata_trt_1$Event, cov.vec = mydata_trt_1[,1:11], mtype = "rand.for", dtype = "b10", time.point = time.point[i])
    targets_1 = create.imp.dr.brier.km(obs = mydata_trt_0$Time, delta = mydata_trt_0$Event, cov.vec = mydata_trt_0[,1:11], mtype = "rand.for", dtype = "b10", time.point = time.point[i])
  ...
  predict_DR_DL_mydata_median_survival_trt_1 <- rep(NA, dim(mydata$X)[1])
  predict_DR_DL_mydata_median_survival_trt_0 <- rep(NA, dim(mydata$X)[1])
  for (j in 1:dim(mydata$X)[1]){
    predict_DR_DL_mydata_median_survival_trt_1_index <- which(abs(DR_DL_mydata_trt_1_survival_prob[j,]-0.5) == min(abs(DR_DL_mydata_trt_1_survival_prob[j,]-0.5)))
    
    predict_DR_DL_mydata_median_survival_trt_1[j] <- (1:50)[predict_DR_DL_mydata_median_survival_trt_1_index]
    
    predict_DR_DL_mydata_median_survival_trt_0_index <- which(abs(DR_DL_mydata_trt_0_survival_prob[j,]-0.5) == min(abs(DR_DL_mydata_trt_0_survival_prob[j,]-0.5)))
    
    predict_DR_DL_mydata_median_survival_trt_0[j] <- (1:50)[predict_DR_DL_mydata_median_survival_trt_0_index]
  }
  ...
  DR_DL_ate <- tibble(predict_mst_trt_2 = predict_DR_DL_mydata_median_survival_trt_1, 
                        predict_mst_trt_1 = predict_DR_DL_mydata_median_survival_trt_0,
                        p1_category = mydata$p1_category) %>% 
    group_by(p1_category) %>%
